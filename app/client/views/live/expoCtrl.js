angular.module('reg')
  .controller('ExpoCtrl', [
    ...
    function($rootScope, $scope, $timeout, $window, $document, $timeout, $http, $location, Utils, EVENT_INFO){
      ...
      var originalSchedulePosition,
          countdownPosition;

      $scope.tintOpacity = 0;

      if ($rootScope.fromState.name === '') {
        originalSchedulePosition = document.getElementsByClassName('schedule-box')[0].getBoundingClientRect().top - 100;
        countdownPosition = document.getElementsByClassName('countdown-container')[0].getBoundingClientRect().top - 75;
      } else {
        originalSchedulePosition = document.getElementsByClassName('schedule-box')[0].getBoundingClientRect().top;
        countdownPosition = document.getElementsByClassName('countdown-container')[0].getBoundingClientRect().top + 25;
      }
